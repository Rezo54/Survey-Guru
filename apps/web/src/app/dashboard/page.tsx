...
const metrics = [
    ['Active projects', '12', 'Demo portfolio figure'],
    ['Locations surveyed', '1,846', 'Demo planning figure'],
    ['High-potential areas', '7', 'Demo opportunity figure'],
];
            ...
            <div className={styles.shell}>
                ...
                <section className={styles.main}>
                    ...
                    <div className={styles.content}>
                        ...
                        <section className={styles.metrics}>{metrics.slice(0,2).map(([l,v,n])=><article className={styles.metric} key={l}><span>{l}</span><strong>{v}</strong><small>{n}</small></article>)}<article className={styles.metric}><span>Field coverage</span><ProjectSummaryCheckpoint mode="metric"/><small>Live confirmed project streets</small></article>{metrics.slice(2).map(([l,v,n])=><article className={styles.metric} key={l}><span>{l}</span><strong>{v}</strong><small>{n}</small></article>)}</section>
                        ...
                        <section className={styles.heroGrid}><article className={styles.panel}><div className={styles.panelHead}><div><p className={styles.eyebrow}>Live market picture</p><h2>Soweto Retail Universe</h2></div><span className={styles.status}><ProjectSummaryCheckpoint mode="badge"/></span></div><ProjectSummaryCheckpoint /><div className={styles.map}><DashboardCoverageMap /><Link className={styles.mapCta} href="/projects/demo/map">Explore authoritative coverage →</Link></div></article>
                        ...
                        <section className={styles.lower}><article className={`${styles.panel} ${styles.mini}`}><p className={styles.eyebrow}>Coverage intelligence</p><h3>Understand what has been searched. See what is next.</h3><div className={styles.stats}><div><strong>Demo</strong><span>Outstanding-distance model</span></div><div><strong>Demo</strong><span>QA-volume model</span></div><div><ProjectSummaryCheckpoint mode="metric"/><span>Live confirmed streets</span></div></div><div className={styles.progress}><i/></div><Link className={styles.link} href="/projects/demo/map">Open Project Map & Coverage →</Link></article>
