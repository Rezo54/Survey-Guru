...
export default function SignInPage() {
  ...
  async function submit(e: FormEvent) {
    e.preventDefault(); if (busy) return; let navigating=false; setBusy(true); setMessage('');
    try {
      ...
      else { await signInWithEmailAndPassword(auth,email,password); const me = await api<{ authority:{ permissions:string[] } }>('/me'); try { window.localStorage.setItem('survey-guru:theme','dark'); } catch {} document.documentElement.dataset.theme='dark'; const p = me.authority.permissions; setMessage('Opening your workspace…'); window.location.assign(p.includes('report.read') ? '/dashboard' : p.includes('qa.review') ? '/qa' : '/field'); navigating=true; }
    } catch(e) { setMessage((e as Error).message); } finally { if (!navigating) setBusy(false); }
  }
