...
export default function StoreCaptureForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const identityQuestions = formConfig.form.questions.filter((question) => question.additionalRow && question.type === 'select' && /brand|product|size|pack/i.test(`${question.id} ${question.label}`));
    const productKeys = additionalRows.map((rowId) => identityQuestions.map((question) => String(form.get(`${question.id}-${rowId}`) ?? '').trim().toLocaleLowerCase()).join('\u0000')).filter((key) => key && !key.split('\u0000').some((value) => !value));
    if (new Set(productKeys).size !== productKeys.length) return setMessage('The same brand, product and pack size cannot be captured twice. Select a different product combination.');
  ...
  function additionalQuestionInput(question: ProjectQuestion, rowId: number) {
    const fieldName = `${question.id}-${rowId}`;
    if (question.type !== 'select' && /price|cost/i.test(question.id+' '+question.label)) return <PriceInput name={fieldName} required={question.required} value={additionalValues[fieldName] ?? ''} onValueChange={value=>setAdditionalValues(current=>({...current,[fieldName]:value}))}/>;
    if (question.type !== 'select') return <input name={fieldName} required={question.required} type="text" inputMode={question.type === 'number' ? 'decimal' : undefined} placeholder={question.type === 'number' ? 'Use comma or full stop for decimals' : undefined} value={additionalValues[fieldName] ?? ''} onChange={(event) => setAdditionalValues((current) => ({ ...current, [fieldName]: event.target.value }))}/>;
    const identityQuestions = formConfig.form.questions.filter((item) => item.additionalRow && item.type === 'select' && /brand|product|size|pack/i.test(`${item.id} ${item.label}`));
    const options = question.options.filter((option) => {
      if (!identityQuestions.includes(question)) return true;
      const candidate = identityQuestions.map((item) => item.id === question.id ? option : additionalValues[`${item.id}-${rowId}`] ?? '');
      if (candidate.some((value) => !value)) return true;
      const key = candidate.map((value) => value.trim().toLocaleLowerCase()).join('\u0000');
      return !additionalRows.some((otherRowId) => otherRowId !== rowId && identityQuestions.map((item) => additionalValues[`${item.id}-${otherRowId}`] ?? '').map((value) => value.trim().toLocaleLowerCase()).join('\u0000') === key);
    });
    return <select name={fieldName} required={question.required} value={additionalValues[fieldName] ?? ''} onChange={(event) => setAdditionalValues((current) => ({ ...current, [fieldName]: event.target.value }))}><option value="">Select an option</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>;
  }
