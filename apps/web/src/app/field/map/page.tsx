...
export default function FieldMapPage() {
  ...
  return <main className={s.page}><div className={s.shell}>
    ...
    <section className={s.main}><div className={s.workspace}>
      <header className={s.header}><div><p className={s.eyebrow}>Survey Guru · Field intelligence</p><h1 className={s.title}>Search what remains</h1><p className={s.subtle}>Your map is the work surface. Search evidence becomes authoritative only after reconciliation.</p></div><span className={s.status}>● Device GPS</span></header>
      ...
      <section className={s.policy}><div><span>Coverage policy</span><strong>Browser foreground · Installed Android/iPhone background</strong></div><div><span>GPS</span><strong>See tracking status above</strong></div><div><span>Sync queue</span><strong>Session-backed above</strong></div></section>
      ...
      <nav className={s.nav} aria-label="Field navigation"><Link href="/field">Field Today</Link><a className={s.active} href="#search-controls">Field Live Map</a></nav>
    </div></section>
  </div></main>;
}
