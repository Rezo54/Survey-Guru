  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!user) { if (!cancelled) setResult({ error: 'not_signed_in', message: 'Sign in required for live assignment data.' }); return; }
        const token = await user.getIdToken();
        const apiOrigin = process.env.NEXT_PUBLIC_SURVEY_GURU_API_URL ?? 'http://127.0.0.1:8080';
        const response = await fetch(`${apiOrigin}/api/v1/projects/prj_soweto_retail_universe/assignments/today`, { headers: { Authorization: `Bearer ${token}` } });
