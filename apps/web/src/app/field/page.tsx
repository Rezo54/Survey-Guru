...
export default function FieldPage() {
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <section className={styles.main}><div className={styles.workspace}>
      ...
      <section className={styles.workGrid}><article className={styles.card}><div className={styles.cardHeader}><div><p className={styles.eyebrow}>Work queue</p><h2>Assignments</h2></div><span className={styles.pill}>3 active</span></div><div className={styles.list}>{assignments.map(([place,type,time,state],i)=><div className={styles.assignment} key={place}><div className={styles.order}>{i+1}</div><div><strong>{place}</strong><span>{type} · {time}</span></div><em className={state==='Priority'?styles.priority:''}>{state}</em></div>)}</div></article>
