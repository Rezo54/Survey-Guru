...
test('validates a practical development capture polygon', () => {
  const result = validateProjectSetup({
    ...
    timeZone: 'Africa/Johannesburg', formTemplateId: 'STANDARD_FMCG', questions: [], productCatalogue: [{ brand: 'Albany', product: '700g White', active: true, displayOrder: 1 }],
    boundary: [
  ...
  assert.equal(result.productCatalogue[0]?.product, '700g White');
  assert.ok(result.areaSquareKm > 0.2 && result.areaSquareKm < 0.4);
});

test('rejects boundaries that cannot form an area', () => {
  assert.throws(() => validateProjectSetup({ name: 'Test project', areaName: 'Test area', timeZone: 'Africa/Johannesburg', formTemplateId: 'STANDARD_FMCG', questions: [], productCatalogue: [], boundary: [{ latitude: 1, longitude: 1 }, { latitude: 1, longitude: 2 }] }), ProjectSetupValidationError);
});
...
test('rejects an excessively broad development polygon', () => {
  assert.throws(() => validateProjectSetup({
    ...
    timeZone: 'Africa/Johannesburg', formTemplateId: 'STANDARD_FMCG', questions: [], productCatalogue: [],
...
test('validates an area timezone and custom select question', () => {
  const result = validateProjectSetup({
    ...
    questions: [{ id: 'storeType', label: 'Store type', type: 'select', required: true, options: ['Kiosk', 'Supermarket'] }], productCatalogue: [],
...
test('rejects invalid timezones and incomplete select questions', () => {
  ...
  assert.throws(() => validateProjectSetup({ name: 'Test project', areaName: 'Test area', timeZone: 'Africa/Nowhere', questions: [], productCatalogue: [], boundary }), /timezone/);
  assert.throws(() => validateProjectSetup({ name: 'Test project', areaName: 'Test area', timeZone: 'Africa/Johannesburg', questions: [{ id: 'channel', label: 'Channel', type: 'select', options: ['Only one'] }], productCatalogue: [], boundary }), /at least two/);
});
