...
export function registerStoreCaptureRoutes(app: FastifyInstance): void {
  ...
  app.post<{ Params: { captureId: string } }>('/api/v1/store-captures/:captureId/submit', async (request) => {
    ...
    const repeatableIdentityQuestions = configuredFormQuestions.filter((value) => value && typeof value === 'object' && (value as Record<string, unknown>).additionalRow === true && (value as Record<string, unknown>).type === 'select' && /brand|product|size|pack/i.test(`${String((value as Record<string, unknown>).id ?? '')} ${String((value as Record<string, unknown>).label ?? '')}`)) as Array<Record<string, unknown>>;
    const repeatableRows = Math.max(0, ...repeatableIdentityQuestions.map((question) => Array.isArray(draft.answers[String(question.id)]) ? (draft.answers[String(question.id)] as unknown[]).length : 0));
    const repeatableProductKeys = Array.from({ length: repeatableRows }, (_, index) => repeatableIdentityQuestions.map((question) => String((draft.answers[String(question.id)] as unknown[] | undefined)?.[index] ?? '').trim().toLocaleLowerCase()).join('\u0000')).filter((key) => key && !key.split('\u0000').some((part) => !part));
    if (new Set(repeatableProductKeys).size !== repeatableProductKeys.length) issues.push('The same brand, product and pack size cannot be captured more than once.');
