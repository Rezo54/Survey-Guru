...
app.addHook('onRequest', async (request, reply) => {
  ...
  if (request.method === 'OPTIONS') {
    if (origin !== allowedWebOrigin) return reply.code(403).send({ error: 'origin_not_allowed', message: 'The request origin is not authorised for this API.' });
...
app.get('/api/v1/me', async (request) => {
  ...
  return { identity, authority: { status: 'authorised', workspaceMembership: { id: authority.membershipId, workspaceId: authority.workspaceId, roleKey: authority.roleKey }, permissions: [...authority.permissions], projectIds: [...authority.projectIds], assignmentIds: [...authority.assignmentIds], resourceScope: 'workspace' } };
});
...
app.get<{ Params: { projectId: string } }>('/api/v1/projects/:projectId/summary', async (request) => {
  ...
  requirePermission(authority, 'project.read'); requireProjectScope(authority, request.params.projectId);
...
app.get<{ Params: { projectId: string } }>('/api/v1/projects/:projectId/assignments/today', async (request) => {
  const identity = await verifyRequestIdentity(request); const authority = await resolveAuthority(identity);
  requirePermission(authority, 'assignment.read'); requireProjectScope(authority, request.params.projectId);
  const { firestore } = getFirebaseAdminServices();
  const snapshot = await firestore.collection('assignments').where('workspaceId', '==', authority.workspaceId).where('projectId', '==', request.params.projectId).where('assignedUserId', '==', identity.uid).where('status', '==', 'active').get();
...
app.post<{ Params: { assignmentId: string } }>('/api/v1/assignments/:assignmentId/search-session', async (request) => {
  const identity = await verifyRequestIdentity(request); const authority = await resolveAuthority(identity);
  requirePermission(authority, 'field.capture'); requireAssignmentScope(authority, request.params.assignmentId);
  const { firestore } = getFirebaseAdminServices();
  const assignment = await firestore.collection('assignments').doc(request.params.assignmentId).get();
  ...
  if (typeof projectId !== 'string') throw new AuthorisationError('Assignment project scope is invalid.');
  ...
  if (!existing.exists) {
    await sessionRef.set({ workspaceId: authority.workspaceId, projectId, assignmentId: assignment.id, userId: identity.uid, teamId: assignment.get('teamId'), areaName: assignment.get('areaName'), state: 'READY', coverageState: 'UNCOVERED', searchedKm: 0, partialKm: 0, unknownKm: assignment.get('outstandingKm') ?? 0, queuedEvidenceCount: 0, environment: process.env.SURVEY_GURU_ENV ?? 'local', updatedAt: new Date().toISOString() });
  }
  const session = await sessionRef.get();
  return { searchSession: { id: session.id, ...session.data() }, authority: { permission: 'field.capture', assignmentScoped: true, projectScoped: true } };
});

const port = Number(process.env.PORT ?? 8080); const host = process.env.HOST ?? '127.0.0.1';
