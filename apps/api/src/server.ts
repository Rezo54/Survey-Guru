...
app.addHook('onRequest', async (request, reply) => {
  ...
  if (origin === allowedWebOrigin) {
    reply.header('Access-Control-Allow-Origin', allowedWebOrigin); reply.header('Vary', 'Origin'); reply.header('Access-Control-Allow-Headers', 'Authorization, Content-Type'); reply.header('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
  }
...
app.get<{ Params: { sessionId: string } }>('/api/v1/search-sessions/:sessionId', async (request) => {
  ...
  const { firestore } = getFirebaseAdminServices(); const session = await firestore.collection('searchSessions').doc(request.params.sessionId).get();
  ...
  return { searchSession: { id: session.id, ...session.data() }, authority: { permission: 'field.capture', assignmentScoped: true, projectScoped: true, identityScoped: true } };
});
